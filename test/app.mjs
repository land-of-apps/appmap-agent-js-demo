...
const requestAsync = (options) =>
  new Promise((resolve, reject) => {
    ...
    req.on("response", (res) => {
      ...
      res.on("end", () => {
        resolve({ status: res.statusCode, body : body === "" ? null : JSON.parse(body) });
      });
    });
    req.end();
  });
...
describe("GET /signup", async () => {
  ...
  it("should return 200 when email is unique", async () => {
    ...
    assert.deepEqual(
      await runAsync(createApp(db), "POST", path),
...
describe("GET /signin", async () => {
  ...
  it("should return 400 when user is missing", async () => {
    ...
    await initializeDBAsync(db);
    assert.deepEqual(
      await runAsync(createApp(db), "GET", path),
      {status:400, body:null},
    );
  });
  it("should return 200 when user exists", async () => {
    ...
    assert.deepEqual(
      await runAsync(createApp(db), "GET", path),
      {status:200, body:{email:"john.doe@mail", name:"john"}},
    );
  });
});
